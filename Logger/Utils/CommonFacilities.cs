...
namespace CommonFacilities
{
	...
	{	
		...
		/// <returns></returns>
		static public string ListToString(List<int> list, string delimiter, string quote)
		{
			string strg = "";
			for (int i = 0; i < list.Count; ++i)
				if (i < list.Count - 1)
					strg += quote + list[i].ToString() + quote + delimiter;
				else
					strg += quote + list[i].ToString() + quote;
			return strg;
		}
		...
		{
			// перекодируем в принятую на сервере кодировку
			return Encoding.GetEncoding(dest_encoding).GetString(
				Encoding.Convert(Encoding.GetEncoding(src_encoding),
				                 Encoding.GetEncoding(dest_encoding),
				                 Encoding.GetEncoding(src_encoding).GetBytes(strg)
				                ));
		}
